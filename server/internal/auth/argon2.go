...
func CheckPassword(candidatePlain PlaintextPassword, hash []byte, salt, paramsJSON []byte) (bool, error) {
	...
	candidate := argon2.IDKey([]byte(candidatePlain), salt, params.Time, params.Memory, params.Threads, params.KeyLen)
...
func EncryptPassword(plaintextPasswd PlaintextPassword, params Argon2Params) ([]byte, []byte, error) {
	...
	return argon2.IDKey([]byte(plaintextPasswd), salt[:], params.Time, params.Memory, params.Threads, params.KeyLen), salt[:], nil
}

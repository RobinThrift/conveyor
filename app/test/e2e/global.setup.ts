...
setup("setup", async ({ page }) => {
    ...
    await page.getByRole("textbox", { name: "Password" }).fill("pls-change-e2e-tests")
    await page.getByRole("button", { name: "Authenticate" }).click()

    await page.getByText("Current PasswordNew").click()
    await page.getByRole("textbox", { name: "Current Password" }).click()
    await page.getByRole("textbox", { name: "Current Password" }).fill("pls-change-e2e-tests")
    await page.getByRole("textbox", { name: "Current Password" }).press("Tab")
    await page.getByRole("textbox", { name: "New Password", exact: true }).fill("e2e-tests")
    await page.getByRole("textbox", { name: "New Password", exact: true }).press("Tab")
    await page.getByRole("textbox", { name: "Repeat new Password" }).fill("e2e-tests")

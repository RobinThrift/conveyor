...
export class EncryptedKVStore<Items extends Record<string, unknown>> implements KVStore<Items> {
    ...
    constructor({
        ...
        this._kv = kv
        this._crypto = crypto
        this._serialize = serialize ?? ((d: any) => encodeText(JSON.stringify(d)))
        this._deserialize = deserialize
    }

    public static ErrGetItem = createErrType("EncryptedKVStore", "error getting item")
    public async getItem<K extends keyof Items>(
        ...
        let [decrypted, decryptionErr] = await this._crypto.decryptData(new Uint8Array(item))
        if (decryptionErr) {
            return wrapErr`${new EncryptedKVStore.ErrGetItem()}: error decrypting data: ${decryptionErr}`
        }

        let [deserialized, desererlisationErr] = this._deserialize(new Uint8Array(decrypted))
    ...
    public static ErrSetItem = createErrType("EncryptedKVStore", "error setting item")
    public async setItem<K extends keyof Items>(
        ctx: Context,
        key: K,
        value: Items[K],
    ): AsyncResult<void> {
        let [encrypted, encryptionErr] = await this._crypto.encryptData(this._serialize(value))
        if (encryptionErr) {
    ...
    public static ErrRemoveItem = createErrType("EncryptedKVStore", "error removing item")
    ...
    public static ErrClear = createErrType("EncryptedKVStore", "error clearing items")

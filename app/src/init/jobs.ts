...
export function initJobs({ jobCtrl, rootStore }: { jobCtrl: JobController; rootStore: RootStore }) {
    let syncJob = new SyncJob(rootStore.dispatch)

    jobCtrl.scheduleJob(syncJob, new EventJobTrigger(globalThis as WorkerGlobalScope, "online"))

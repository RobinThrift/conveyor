...
suite.concurrent("external/browser/BrowserIndexedDB", { timeout: 5000 }, () => {
    let [ctx, cancel] = BaseContext.withCancel()
    afterAll(() => cancel())

    test("migrations", { timeout: 5000 }, async ({ onTestFinished }) => {
        let idb = new BrowserIndexedDB()
    ...
    test("insert error", { timeout: 5000 }, async ({ onTestFinished }) => {
        ...
        await assertOkResult(
            idb.open(ctx, "tests.insert_error", [
                async (db) => {
                    db.createObjectStore("items", {
                        keyPath: "title",
                    })
    ...
    test("binary data", { timeout: 5000 }, async ({ onTestFinished }) => {
        ...
        await assertOkResult(
            idb.open(ctx, "tests.binary_data", [
                async (db) => {
                    db.createObjectStore("items", {
                        keyPath: "title",
                    })

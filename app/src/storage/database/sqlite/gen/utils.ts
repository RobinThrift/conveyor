...
export function columnNameToFieldName(column: string) {
    return column.toLowerCase().replace(/([_][a-z])/g, group => group.toUpperCase().replace("_", ""));
}

export function mapRowToObj<T extends object>(row: Record<string, SqlValue | boolean>, overrides: Partial<Record<keyof T, (v: any) => any>> = {}): T {
    let obj = {} as T;
    for (let key in row) {

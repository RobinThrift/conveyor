...
export async function getAttachment(database: Database, args: GetAttachmentArgs, abort?: AbortSignal): Promise<GetAttachmentRow | null> {
...
export async function getAttachmentByFilepath(database: Database, args: GetAttachmentByFilepathArgs, abort?: AbortSignal): Promise<GetAttachmentByFilepathRow | null> {
    let result = await database.queryOne(getAttachmentByFilepathQuery, [args.filepath], abort);
    if (result === undefined) {
        return null;
    }
    return mapRowToObj<GetAttachmentByFilepathRow>(result, { createdAt: dateFromSQLite });
}
...
export async function countAttachments(database: Database, abort?: AbortSignal): Promise<CountAttachmentsRow | null> {
...
export async function listAttachments(database: Database, args: ListAttachmentsArgs, abort?: AbortSignal): Promise<ListAttachmentsRow[]> {
...
export async function createAttachment(database: Database, args: CreateAttachmentArgs, abort?: AbortSignal): Promise<CreateAttachmentRow | null> {
    let result = await database.queryOne(createAttachmentQuery, [args.publicId, args.filepath, args.originalFilename, args.contentType, args.sizeBytes, args.sha256], abort);
    if (result === undefined) {
        return null;
    }
...
export async function deleteAttachments(database: Database, args: DeleteAttachmentsArgs, abort?: AbortSignal) {
    let deleteAttachmentsQueryWithSliceParams = deleteAttachmentsQuery.replace("/*SLICE:ids*/?", [
        ...Array(args.ids.length).keys().map(i => `?${i + 1}`)
    ].join(","));
...
export async function listAttachmentsForMemo(database: Database, args: ListAttachmentsForMemoArgs, abort?: AbortSignal): Promise<ListAttachmentsForMemoRow[]> {
    let result = await database.query(listAttachmentsForMemoQuery, [args.memoId], abort);
    return result.map(row => mapRowToObj<ListAttachmentsForMemoRow>(row, { createdAt: dateFromSQLite }));
}
...
export async function createMemoAttachmentLink(database: Database, args: CreateMemoAttachmentLinkArgs, abort?: AbortSignal) {
...
export async function deleteAllMemoAttachmentLinks(database: Database, args: DeleteAllMemoAttachmentLinksArgs, abort?: AbortSignal) {
...
export async function deleteMemoAttachmentLinks(database: Database, args: DeleteMemoAttachmentLinksArgs, abort?: AbortSignal) {
    let deleteMemoAttachmentLinksQueryWithSliceParams = deleteMemoAttachmentLinksQuery.replace("/*SLICE:attachment_ids*/?", [
        ...Array(args.attachmentIds.length).keys().map(i => `?${i + 2}`)
    ].join(","));
    await database.exec(deleteMemoAttachmentLinksQueryWithSliceParams, [args.memoId, ...args.attachmentIds], abort);
}
...
export async function listUnusedAttachments(database: Database, abort?: AbortSignal): Promise<ListUnusedAttachmentsRow[]> {
    let result = await database.query(listUnusedAttachmentsQuery, [], abort);
    return result.map(row => mapRowToObj<ListUnusedAttachmentsRow>(row, { createdAt: dateFromSQLite }));
}
